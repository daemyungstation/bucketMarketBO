...
<body>
                                        ...
                                        <select class="select" name="searchBrdCmnComIdx" id="searchBrdCmnComIdx">
                                            ...
                                            <c:if test="${not empty codeList }">
                                                <c:forEach var="row" items="${codeList }" varStatus="i">
                                                    <option value="<c:out value="${row.CMN_COM_IDX }"/>" <c:if test="${row.CMN_COM_IDX eq commandMap.searchBrdCmnComIdx }">selected="selected"</c:if>>
                                ...
                                <c:if test="${not empty topList }">
                                    <c:forEach var="row" items="${topList }" varStatus="i">
                                        <tr>
                                            ...
                                            <td>
                                                <ui:formatDate value="${row.BRD_NTS_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss" />
                                            </td>
                                            <td>
                                                <c:out value="${row.BRD_NTS_READ_CNT }" />
                                            </td>
                                            <td>
                                                <c:out value="${row.BRD_NTS_TOP_YN eq 'Y'?'상단공지':'일반공지' }" />
                                            </td>
                                            <td>
                                                <c:out value="${row.BRD_NTS_REG_ID }" />
                                                (
                                                <c:out value="${row.BRD_NTS_REG_NM }" />
                                                )
                                            </td>
                                            <td>
    ...
    <script>
        $(function() {
            ...
            <%-- 키워드 검색 필드 엔터 이벤트 --%>
            $wrapper.on("keydown", "#searchBrdNtsTitleAndContents", function(e) {
                if (e.keyCode == 13) {
