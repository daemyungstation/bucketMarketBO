                ...
                <div class="container">
                    ...
                    <div class="tab-list">
                        <ul>
                            ...
                            <li class="col col-1-5 in" style="width: 18%"><a href="javascript:;" class="term-type-tab text-center" data-tab_type="ProductType"><span>상품유형</span></a></li>
                    ...
                    <table cellspacing="0" class="table-col table-b">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th colspan="4">월지원</th>
                                <th colspan="4">즉발(채권)</th>
                                <th colspan="4">즉발(포인트)</th>
                                <th colspan="4">할부지원</th>
                                <th colspan="4">렌탈</th>
                            </tr>
                            <tr>
                            ...
                            <c:if test="${not empty list }">
                                <c:forEach var="row" items="${list }" varStatus="stat">
                                    <tr <c:if test="${stat.last }">class="total"</c:if>>
                                        ...
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MONTHLY_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MONTHLY_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MONTHLY_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MONTHLY_C }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_Y_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_Y_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_Y_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_Y_C }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_N_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_N_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_N_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.IMMEDIATELY_N_C }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.INSTALLMENT_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.INSTALLMENT_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.INSTALLMENT_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.INSTALLMENT_C }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.RENTAL_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.RENTAL_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.RENTAL_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.RENTAL_C }" type="number"/></td>
                                    </tr>
                                </c:forEach>
                            </c:if>
    ...
    <script>
        $(function() {
            ...
            $wrapper.on("click", "#search", function() {
                ...
                $searchForm.postSubmit($.action.view("ProductType", "statsJoinAnalysis"));
            });
            <%-- 엑셀 다운로드 --%>
            $wrapper.on("click", "#goExcelDownload", function() {
                $frm.postSubmit($.action.exceldownload("ProductType", "statsJoinAnalysis"));
            });
        });
    </script>
</body>
