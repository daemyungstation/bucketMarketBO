        ...
        <div id="wrapper">
            ...
            <div id="contents">
                <div class="container">
                    ...
                    <div class="tab-list">
                        <ul>
                            <li class="col col-1-5 in" style="width: 18%"><a href="javascript:;" class="term-type-tab text-center" data-tab_type="Sex"><span>성별</span></a></li>
                            ...
                            <c:if test="${not empty list }">
                                <c:forEach var="row" items="${list }" varStatus="stat">
                                    <tr <c:if test="${stat.last }">class="total"</c:if>>
                                        ...
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MALE_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MALE_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MALE_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.MALE_C }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FEMALE_R }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FEMALE_W }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FEMALE_J }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FEMALE_C }" type="number"/></td>
                                    </tr>
                                </c:forEach>
                            </c:if>
    ...
    <script>
        $(function() {
            ...
            $wrapper.on("click", "#search", function() {
                ...
                $searchForm.postSubmit($.action.view("Sex", "statsJoinAnalysis"));
            });
            <%-- 엑셀 다운로드 --%>
            $wrapper.on("click", "#goExcelDownload", function() {
                $frm.postSubmit($.action.exceldownload("Sex", "statsJoinAnalysis"));
            });
        });
    </script>
</body>
