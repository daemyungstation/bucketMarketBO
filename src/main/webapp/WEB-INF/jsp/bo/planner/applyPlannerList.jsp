...
<body>
                        ...
                        <table cellspacing="0" class="table-row table-a" style="margin-bottom: 0;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="searchRdpMstMemNm" id="searchRdpMstMemNm" class="text xlarge" value="<c:out value="${commandMap.searchRdpMstMemNm }"/>" />
                                    </td>
                                    <th>
                                        <span>이메일주소</span>
                                    </th>
                                    <td>
                                        <input type="text" name="searchRdpMstMail" id="searchRdpMstMail" class="text xlarge" value="<c:out value="${commandMap.searchRdpMstMail }"/>" />
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                                    ...
                                    <td>
                                        <input type="text" name="searchRdpMstHp3" id="searchRdpMstHp3" class="text xlarge" value="<c:out value="${commandMap.searchRdpMstHp3 }"/>" />
                                    </td>
                                    <th>
                                        <span>소속</span>
                                    </th>
                                ...
                                <tr>
                                    ...
                                    <td colspan="3">
                                        ...
                                        <input type="radio" id="searchRdpMstState1" class="radio" name="searchRdpMstState" value="<c:out value="${Code.PLANNER_STATE_APPLY }" />" <c:if test="${commandMap.searchRdpMstState eq Code.PLANNER_STATE_APPLY }">checked="checked"</c:if>/>
                                        <label for="searchRdpMstState1">신청</label>
                                        <input type="radio" id="searchRdpMstState2" class="radio" name="searchRdpMstState" value="<c:out value="${Code.PLANNER_STATE_REJECT }" />" <c:if test="${commandMap.searchRdpMstState eq Code.PLANNER_STATE_REJECT }">checked="checked"</c:if>/>
                                        <label for="searchRdpMstState2">반려</label>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <form id="frm" name="frm">
                        ...
                        <div class="grid section-button-list">
                            ...
                            <div class="col-1-2 text-right">
                                ...
                                <a href="javascript:;" id="approvalAll" class="button small primary">
                                    <span>승인처리</span>
                                </a>
                            </div>
                        </div>
                                    ...
                                    <c:when test="${not empty list}">
                                        <c:forEach var="row" items="${list }" varStatus="i">
                                            <tr>
                                                ...
                                                <td>
                                                    <ui:formatDate value="${row.RDP_MST_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss"/>
                                                </td>
                                                <td>
    ...
    <script>
        $(function() {
            ...
            // 승인 처리
            $wrapper.on("click", "#approvalAll", function() {
                var $target = $frm.find(".check-row:checked");
                if ($target.length == 0) {
                    alert("승인 처리할 플래너를 선택해 주십시오.");
                    return;
                }
                if (confirm("승인 처리 하시겠습니까?")) {
                    ...
                    $.postSyncJsonAjax($.action.ajax("ApproveAll"), {"PARAMS" : JSON.stringify(array)}, function(data) {
                        if (data.isSuccess) {
                            alert("승인이 완료되었습니다.");
                            goPage(1);
                        } else {
