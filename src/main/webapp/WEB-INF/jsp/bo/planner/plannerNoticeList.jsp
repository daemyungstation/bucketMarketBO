            ...
            <div id="contents">
                ...
                <div class="container">
                        ...
                        <table cellspacing="0" class="table-row table-a" style="margin-bottom: 0;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="radio" id="searchRdpNtsUseYn0" class="radio" name="searchRdpNtsUseYn" value="" <c:if test="${empty commandMap.searchRdpNtsUseYn }">checked="checked"</c:if>/>
                                        <label for="searchRdpNtsUseYn0">전체</label>
                                        <input type="radio" id="searchRdpNtsUseYn1" class="radio" name="searchRdpNtsUseYn" value="Y" <c:if test="${commandMap.searchRdpNtsUseYn eq 'Y' }">checked="checked"</c:if>/>
                                        <label for="searchRdpNtsUseYn1">노출</label>
                                        <input type="radio" id="searchRdpNtsUseYn2" class="radio" name="searchRdpNtsUseYn" value="N" <c:if test="${commandMap.searchRdpNtsUseYn eq 'N' }">checked="checked"</c:if>/>
                                        <label for="searchRdpNtsUseYn2">노출안함</label>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <form id="frm" name="frm">
                        <input type="hidden" name="RDP_NTS_IDX" value="" />
                                ...
                                <c:if test="${not empty topList }">
                                    <c:forEach var="row" items="${topList }" varStatus="i">
                                        <tr>
                                            ...
                                            <td class="left">
                                                <a href="javascript:;" class="list-title" data-rdp_nts_idx="<c:out value="${row.RDP_NTS_IDX }"/>">
                                                    <c:out value="${row.RDP_NTS_TITLE }" />
                                                </a>
                                            </td>
                                            <td>
                                                <c:out value="${row.RDP_NTS_READ_CNT }" />
                                            </td>
                                            <td>
                                                <ui:formatDate value="${row.RDP_NTS_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss" />
                                            </td>
                                            <td>
                                                <c:out value="${row.RDP_NTS_USE_YN eq 'Y' ? '노출' : '노출안함' }" />
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </c:if>
    ...
    <script>
        $(function() {
            ...
            $wrapper.on("click", ".list-title", function() {
                $frm.find("input[name='RDP_NTS_IDX']").val($(this).data("rdp_nts_idx"));
