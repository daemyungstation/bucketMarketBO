...
<div class="contents-inner" data-cmn_com_idx="<c:out value="${commandMap.CMN_COM_IDX }"/>">
    <h3 class="title">
        <span><c:out value="${commandMap.CMN_COM_PATH }" escapeXml="false"/> 수정</span>
    </h3>
        ...
        <table cellspacing="0" class="table-row table-a">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <input type="radio" name="BNR_MST_USE_YN" id="BNR_MST_USE_YN0" class="radio" value="Y" <c:if test="${info.BNR_MST_USE_YN eq 'Y' }">checked="checked"</c:if>/>
                        <label for="BNR_MST_USE_YN0">사용</label>
                        <input type="radio" name="BNR_MST_USE_YN" id="BNR_MST_USE_YN1" class="radio" value="N" <c:if test="${info.BNR_MST_USE_YN eq 'N' }">checked="checked"</c:if>/>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="BNR_MST_TITLE" id="BNR_MST_TITLE" class="text xlarge" value="<c:out value="${info.BNR_MST_TITLE }"/>" maxlength="70" />
                    </td>
                </tr>
                <tr>
                    <th>
                    ...
                    <td>
                        ...
                        <input type="text" name="BNR_MST_ST_DT" id="BNR_MST_ST_DT" class="text date" value="<ui:formatDate value="${info.BNR_MST_ST_DT }" pattern="yyyy-MM-dd"/>" data-target-end="#BNR_MST_ED_DT" />
                        <select name="BNR_MST_ST_HH" id="BNR_MST_ST_HH" class="select small">
                            <c:forEach var="hh" begin="0" end="23" step="1">
                                ...
                                <c:out value="${hour }"/>
                                <option value="<c:out value="${hour }"/>" <c:if test="${info.BNR_MST_ST_HH eq hour }">selected="selected"</c:if>><c:out value="${hour }"/></option>
                            </c:forEach>
                        </select>
                        <select name="BNR_MST_ST_MM" id="BNR_MST_ST_MM" class="select small">
                            <c:forEach var="mm" begin="0" end="59" step="1">
                                ...
                                <option value="<c:out value="${minute }"/>" <c:if test="${info.BNR_MST_ST_MM eq minute }">selected="selected"</c:if>><c:out value="${minute }"/></option>
                            </c:forEach>
                        </select>
                        ~
                        <a href="javascript:;" class="js-datepickr">
                        ...
                        <input type="text" name="BNR_MST_ED_DT" id="BNR_MST_ED_DT" class="text date" value="<ui:formatDate value="${info.BNR_MST_ED_DT }" pattern="yyyy-MM-dd"/>" data-target-start="#BNR_MST_ST_DT" />
                        <select name="BNR_MST_ED_HH" id="BNR_MST_ED_HH" class="select small">
                            <c:forEach var="hh" begin="0" end="23" step="1">
                                ...
                                <c:out value="${hour }"/>
                                <option value="<c:out value="${hour }"/>" <c:if test="${info.BNR_MST_ED_HH eq hour }">selected="selected"</c:if>><c:out value="${hour }"/></option>
                            </c:forEach>
                        </select>
                        <select name="BNR_MST_ED_MM" id="BNR_MST_ED_MM" class="select small">
                            <c:forEach var="mm" begin="0" end="59" step="1">
                                ...
                                <option value="<c:out value="${minute }"/>" <c:if test="${info.BNR_MST_ED_MM eq minute }">selected="selected"</c:if>><c:out value="${minute }"/></option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <c:forEach var="target" items="${Const.LINK_TARGET }" varStatus="i">
                            <input type="radio" name="BNR_MST_TGT" id="BNR_MST_TGT<c:out value="${i.index }"/>" value="<c:out value="${target.key }"/>" <c:if test="${info.BNR_MST_TGT eq target.key }">checked="checked"</c:if>/>
                            <label for="BNR_MST_TGT<c:out value="${i.index }"/>"><c:out value="${target.value }"/></label>
                        </c:forEach>
                        <br/>
                        <input type="text" placeholder="링크 입력 시 https://를 포함하여 전체 링크를 입력해 주세요"  name="BNR_MST_URL" id="BNR_MST_URL" class="text xlarge" value="<c:out value="${info.BNR_MST_URL }"/>" maxlength="100" <c:if test="${info.BNR_MST_TGT eq Const.LINK_TARGET_NONE }">disabled="disabled"</c:if>/>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="PRD_CTG_IDX">
                            <option value="" <c:if test="${empty info.PRD_CTG_IDX }">selected</c:if>>전체</option>
                            <c:forEach items="${cateList }" var="cateRow" >
                            <option value="<c:out value="${cateRow.PRD_CTG_IDX }"/>" <c:if test="${info.PRD_CTG_IDX eq cateRow.PRD_CTG_IDX}">selected</c:if>><c:out value="${cateRow.PRD_CTG_NM }"/></option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
