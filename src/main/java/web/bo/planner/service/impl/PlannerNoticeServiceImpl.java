...
public class PlannerNoticeServiceImpl implements PlannerNoticeService {
    ...
    public int insertNotice(MultipartHttpServletRequest request, Map<String, Object> commandMap) throws Exception {
        ...
        if (result > 0){
            ...
            alimList = (List<Map<String, Object>>) defaultDAO.selectList("PlannerNotice.selectBasicPlannerList", commandMap);
            System.out.println(alimList);
            
            for(Map<String, Object> row : alimList) {
                String RDP_MST_HP = StringUtil.getString(row, "RDP_MST_HP");
                String RDP_MST_MEM_NM = StringUtil.getString(row, "RDP_MST_MEM_NM");
                String RDP_NTS_TITLE = StringUtil.getString(row, "RDP_NTS_TITLE");
                this.messageService.sendAlimTalk(MessageType.PLANNER_NOTICE_REQUEST, RDP_MST_HP, RDP_MST_MEM_NM, RDP_NTS_TITLE);
            }
        }
