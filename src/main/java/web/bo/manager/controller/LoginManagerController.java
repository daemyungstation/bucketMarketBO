...
public class LoginManagerController {
    ...
    public ModelAndView login(HttpServletRequest request, HttpServletResponse response, Map<String, Object> commandMap) throws Exception {
        ...
        if (SessionsManager.isLogin(request)) {
            ...
            if( SessionsManager.getSessionManager(request).getLOGIN_STATE_CODE() == LoginState.LOGIN_DORMANCY
                && "True".equals(commandMap.get("clearDormancy")) ) {
                mv.addObject("clearDormancy", "True");
            }
    ...
    public ModelAndView loginProc(HttpServletRequest request, HttpServletResponse response, @RequestParams Map<String, Object> commandMap) throws Exception {
        ...
        if (LOGIN_STATE_CODE == LoginState.LOGIN_ACCREDIT) { // 이차 로그인 처리
            ...
            String clearDormancy = StringUtil.getString(commandMap, "clearDormancy", "N");
            // 로그인 처리
            int dormancyDays = 90;
            manager = loginManagerService.loginManagerProc(manager, request, response, dormancyDays, "True".equals(clearDormancy));
            if( manager.getLOGIN_STATE_CODE() == LoginState.LOGIN_DORMANCY) {
                mv.addObject("manager", manager);
                mv.addObject("commandMap", commandMap);
                mv.setViewName("bo/manager/login");
            } else if (manager.getLOGIN_STATE_CODE() == LoginState.LOGIN_ACCREDIT) {// 이차 로그인 인증 페이지
