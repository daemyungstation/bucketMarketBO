...
//            commandMap.put("isSuper", SessionsManager.isSuper());
...
//        commandMap.put("searchStateNm", StringUtil.getString(commandMap, "searchStateNm", Product.DISPLAY_SHOW));
    ...
    public EXCEL productApprovalExcelDownload(HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception {
        ...
        if (list != null) {
            ...
            for(Map<String, Object> row : list) {
                String  dplYn = StringUtil.getString(row, "PRD_MST_DPL_YN", "");
                ...
                if(dplYn.equals("Y")){
                    dplYn = "노출";
                    row.put("PRD_MST_DPL_YN", dplYn);
                } else if(dplYn.equals("N")){
                    dplYn = "노출안함";
                    row.put("PRD_MST_DPL_YN", dplYn);
                } else if(dplYn.equals("S")){
                    dplYn = "일시품절";
                    row.put("PRD_MST_DPL_YN", dplYn);
                }
            }
            excel.put("list", list);
            String [] headerTitle = {"NO",   "제휴업체",      "모델분류",      "지원혜택명",         "모델명",       "공급가",            "승인일",         "상태",        "노출여부",         "상품코드"};
            String [] cellField = {"RNUM",   "VDR_MST_NM", "MODEL_CL_NM", "PRD_MST_SPL_NM", "PRD_MST_MD", "PRD_MST_SPL_PRC", "PRD_MST_APV_DT", "PRD_MST_APV_STATE_NM", "PRD_MST_DPL_YN", "PRD_MST_NO"};
            String [] cellType =  {"Number", "String",     "String",      "String",         "String",     "Dollar",          "Date",           "String",           "String",           "String"};
            String [] colSize = {"50", "80", "150", "150", "80", "50", "50", "50", "50", "100"};
    ...
    /**
     * <pre>
     * 1. MethodName : productApprovalOptionModify
     * 2. ClassName  : ProductApprovalController.java
     * 3. Comment    : 관리자 > 상품관리 > 승인상품 옵션 수정
     * 4. 작성자       : inus
     * 5. 작성일       : 2020. 8. 12.
     * </pre>
     *
     ...
     * @throws Exception
     */
    @RequestMapping(value="/bo/product/productApprovalOptionModifyAjax")
