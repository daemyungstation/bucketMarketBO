	...
	var rFilter = /^(image\/gif|image\/jpg|image\/jpeg|image\/png)$/i;  
    ...
    function html5Upload() {	
    	var tempFile,
    		sUploadURL;
    	var csrfName = sessionStorage.getItem('csrfName'),
            csrfValue = sessionStorage.getItem('csrfValue');
    	sUploadURL = '/common/commonFileEditorHtml5Upload.do?'; 	//upload URL
    	sUploadURL += csrfName + '=' + csrfValue;
    	//파일을 하나씩 보내고, 결과를 받음.
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
                    if(sResString.indexOf("NOTFILE") > -1){
                        alert("이미지 파일이 누락되었습니다.");
                    }else if(sResString.indexOf("NOTALLOW_") > -1){
						var sFileName = sResString.replace("NOTALLOW_", "");
						alert("업로드 불가능한 확장자 입니다.\n첨부파일 확장자를 확인 후 다시 시도 해 주십시오.\n업로드 가능한 확장자("+sFileName+")");
					}else if(sResString.indexOf("NOTSIZE_") > -1){
					    var sFileName = sResString.replace("NOTALLOW_", "");
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 	   var csrfName = sessionStorage.getItem('csrfName'),
 	       csrfValue = sessionStorage.getItem('csrfValue'),
 	       sUrl = '/common/commonFileEditorUpload.do?' + csrfName + '=' + csrfValue;
