            ...
            <div id="contents">
                ...
                <div class="container">
                    ...
                    <form id="frm" name="frm">
                        <input type="hidden" name="AGR_MST_IDX" value="<c:out value="${info.AGR_MST_IDX }"/>"/>
                        <input type="hidden" name="CMN_COM_IDX" value="<c:out value="${commandMap.CMN_COM_IDX }"/>"/>
                        <input type="hidden" name="cPage" value="<c:out value="${commandMap.cPage }"/>"/>
                        <input type="hidden" name="searchAgrMstGbn" value="<c:out value="${commandMap.searchAgrMstGbn }"/>"/>
                        <input type="hidden" name="searchType" value="<c:out value="${commandMap.searchType }"/>"/>
                        ...
                        <table cellspacing="0" class="table-row table-a">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <select name="AGR_MST_TYPE" id="AGR_MST_TYPE" class="select" disabled="disabled">
                                            <option value="">선택</option>
                                            <c:if test="${not empty codes.TERM_CODE }">
                                                <c:forEach var="row" items="${codes.TERM_CODE }" varStatus="i">
                                                    <option value="<c:out value="${row.CMN_COM_IDX }"/>" <c:if test="${row.CMN_COM_IDX eq info.AGR_MST_TYPE }">selected="selected"</c:if>>
                                                        <c:out value="${row.CMN_COM_NM }" />
                                                    </option>
                                                </c:forEach>
                                            </c:if>
                                        </select>
                                        <c:if test="${info.AGR_MST_TYPE eq Code.TERM_PERSONAL_INFORMATION_CODE or info.AGR_MST_TYPE eq Code.TERM_CONTRACT_CODE or info.AGR_MST_TYPE eq Code.TERM_READY_PLANNER_CODE}">
                                            <select name="AGR_MST_GBN" id="AGR_MST_GBN" class="select" disabled="disabled">
                                                <option value="">선택</option>
                                                <c:if test="${info.AGR_MST_TYPE eq Code.TERM_PERSONAL_INFORMATION_CODE }">
                                                    <c:if test="${not empty codes.TERM_PERSONAL_INFORMATION_CODE }">
                                                        <c:forEach var="row" items="${codes.TERM_PERSONAL_INFORMATION_CODE }" varStatus="i">
                                                            <option value="<c:out value="${row.CMN_COM_IDX }"/>" <c:if test="${row.CMN_COM_IDX eq info.AGR_MST_GBN }">selected="selected"</c:if>>
                                                                <c:out value="${row.CMN_COM_NM }" />
                                                            </option>
                                                        </c:forEach>
                                                    </c:if>
                                                </c:if>
                                                <c:if test="${info.AGR_MST_TYPE eq Code.TERM_CONTRACT_CODE }">
                                                ...
                                                <c:if test="${info.AGR_MST_TYPE eq Code.TERM_READY_PLANNER_CODE }">
                                            ...
                                            <c:if test="${fn:contains(Code.TERM_CONTRACT_CONFIRM_FIRST_LIST, info.AGR_MST_GBN) }">
                                                <input type="text" name="AGR_MST_PRD_NO" id="AGR_MST_PRD_NO" class="text" value="<c:out value="${info.AGR_MST_PRD_NO }"/>" maxlengt="5" placeholder="상품코드 입력" disabled="disabled"/>
                                            </c:if>
                                        </c:if>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <ui:formatDate value="${info.AGR_MST_CRT_DAY }" pattern="yyyy.MM.dd" />
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                                    ...
                                    <td>
                                        <label for="AGR_MST_USE_YN0">
                                            <input type="radio" name="AGR_MST_USE_YN" id="AGR_MST_USE_YN0" class="radio" value="Y" <c:if test="${info.AGR_MST_USE_YN eq 'Y' }">checked="checked"</c:if>/>
                                            <span>노출</span>
                                        </label>
                                        <label for="AGR_MST_USE_YN1">
                                            <input type="radio" name="AGR_MST_USE_YN" id="AGR_MST_USE_YN1" class="radio" value="N" <c:if test="${info.AGR_MST_USE_YN eq 'N' }">checked="checked"</c:if>/>
                                            <span>미노출</span>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="AGR_MST_TITLE" id="AGR_MST_TITLE" class="text xlarge" value="<c:out value="${info.AGR_MST_TITLE }"/>" maxlength="70"/>
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                                    ...
                                    <td>
                                        <textarea name="AGR_MST_CONTENTS" id="AGR_MST_CONTENTS" class="textarea xlarge" style="height: 500px; width: 100%"><c:out value="${info.AGR_MST_CONTENTS }"/></textarea>
                                    </td>
                                </tr>
                                <c:if test="${info.AGR_MST_TYPE eq Code.TERM_BUCKET_MARKET_USE or info.AGR_MST_TYPE eq Code.TERM_PERSONAL_INFORMATION_CODE}">
                                    <tr class="prev-tr">
                                        <th>
                                            <span>이전약관</span>
                                        </th>
                                        <td>
                                            <label for="AGR_MST_PRE_YN">
                                                <span>이전약관 노출</span>
                                                <input type="checkbox" name="AGR_MST_PRE_YN" id="AGR_MST_PRE_YN" class="checkbox" value="Y" <c:if test="${info.AGR_MST_PRE_YN eq 'Y' }">checked="checked"</c:if>/>
                                            </label>
                                        </td>
                                    </tr>
                                </c:if>
