...
public class ProductServiceImpl implements ProductService {
    ...
    * 5. 작성일       : 2020. 4. 28.
    ...
    public Map<String, Object> selectBasicProductInfo(Map<String, Object> commandMap) throws Exception {
        ...
        List<Map<String, Object>> prpList = defaultDAO.selectList("BasicProduct.selectPrivateProductPlannerList", commandMap);
        if( prpList != null && prpList.size() > 0 ){
            //이걸 꼭 JSON으로 해야 하는가.
            JSONArray jsonPrpList = JSONUtil.convListMapToJSONArray(prpList);
            returnMap.put("prpListJson",Base64Util.encode(URLEncoder.encode(jsonPrpList.toString(), "utf-8")));
        }
        returnMap.put("prpList", prpList);
        return returnMap;
    }
    ...
    * 5. 작성일       : 2020. 4. 28.
    ...
    public int updateProductDisapproval(MultipartHttpServletRequest request, Map<String, Object> commandMap) throws Exception {
        ...
        result += updatePrivateProductPlannerList(commandMap);
    ...
    private int updatePrivateProductPlannerList(Map<String, Object> commandMap) throws Exception {
        int result = 0;
        result += defaultDAO.delete("BasicProduct.deletePrivateProductPlannerList", commandMap);
        if("Y".equals(StringUtil.getString(commandMap, "PRD_MST_PRP_YN", "N") )) {
            String prpListJsonBase64 = (String)commandMap.get("PRP_LIST_JSON");
            if( StringUtils.isNotEmpty(prpListJsonBase64)) {
                byte [] decodedBase64Array = Base64.getDecoder().decode(prpListJsonBase64);
                String prpListJson = URLDecoder.decode(new String(decodedBase64Array) , "utf-8");
                JSONArray jsonArray = JSONUtil.convStringToJSONArray(prpListJson);
                List<Map<String, Object>> prpListMap = JSONUtil.convJSONArrayToListMap(jsonArray);
                commandMap.put("RDP_MST_IDX_LIST", prpListMap);
                result += defaultDAO.insert("BasicProduct.insertPrivateProductPlannerList", commandMap);
            }
        }
        return result;
    }
    ...
    * 5. 작성일       : 2020. 4. 28.
    ...
    public int insertProductUnion(MultipartHttpServletRequest request, Map<String, Object> commandMap) throws Exception {
        ...
        //TODO: 결합상품하고 승인상품하고 달라야 하는지
        result += updatePrivateProductPlannerList(commandMap);

        commandMap.put("PRD_MST_RDP_CMS_CD", commandMap.get("PRC_RDP_CMS_CD"));
        commandMap.put("PRD_MST_RDP_CMS", commandMap.get("PRC_RDP_CMS"));
        commandMap.put("PRD_MST_RDP_DVD_PAY", commandMap.get("PRC_RDP_DVD_PAY"));
        commandMap.put("PRD_MST_RDP_DVD_CNT", commandMap.get("PRC_RDP_DVD_CNT"));
        result += defaultDAO.insert("BasicProduct.insertProductUnion", commandMap);


        /* 법인 수수료 컬럼 입력 */
        commandMap.put("PRC_PAY_CLS", Code.PLANNER_PAY_CLASS_LEGAL_PERSON);
//        commandMap.put("PRC_RDP_CMS_CD", commandMap.get("PRC_RDP_CMS_CD"));
//        commandMap.put("PRC_RDP_CMS", commandMap.get("PRC_RDP_CMS"));
//        commandMap.put("PRC_RDP_DVD_PAY", commandMap.get("PRC_RDP_DVD_PAY"));
//        commandMap.put("PRC_RDP_DVD_CNT", commandMap.get("PRC_RDP_DVD_CNT"));
        defaultDAO.insert("BasicProduct.insertProductUnionCMS", commandMap);

        /* 개인 수수료 컬럼 입력 */
        commandMap.put("PRC_PAY_CLS", Code.PLANNER_PAY_CLASS_PRIVATE);
        commandMap.put("PRC_RDP_CMS_CD", commandMap.get("PRC_RDP_P_CMS_CD"));
        commandMap.put("PRC_RDP_CMS", commandMap.get("PRC_RDP_P_CMS"));
        commandMap.put("PRC_RDP_DVD_PAY", commandMap.get("PRC_RDP_P_DVD_PAY"));
        commandMap.put("PRC_RDP_DVD_CNT", commandMap.get("PRC_RDP_P_DVD_CNT"));
        defaultDAO.insert("BasicProduct.insertProductUnionCMS", commandMap);

        commandMap.put("CMM_CLE_NM", "FILE");
        commandMap.put("CMM_FLE_TB_NM", "C_PRD_MST");
        result += commonFileService.insertCommonFileList(request, commandMap, UPLOAD_PATH, UPLOAD_ALLOW, UPLOAD_DATE_FOLDER_FLAG);
        result += defaultDAO.update("BasicProduct.updateProductUnionEdit", commandMap);
        result += productImageService.updateProductImage((MultipartHttpServletRequest) request, commandMap);
    ...
    * 3. Comment    : 관리자 > 상품관리 > 결합상품 추천 순서 수정
    ...
    public int updateProductSuggest(Map<String, Object> commandMap) throws Exception {
        ...
        Map<String, Object> dataMap =  new HashMap<>();
        dataMap.put("PRD_MST_NO", StringUtil.getString(commandMap, "PRD_MST_NO", ""));
        dataMap.put("PRD_MST_SUGGEST", StringUtil.getString(commandMap, "PRD_MST_SUGGEST", ""));
        
        result += defaultDAO.update("BasicProduct.updateProductSuggest", dataMap);
