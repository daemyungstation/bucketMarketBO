                ...
                <div class="container">
                    ...
                    <div class="tab-list">
                        <ul>
                            ...
                            <li class="col col-1-5 in" style="width: 18%"><a href="javascript:;" class="term-type-tab text-center" data-tab_type="Age"><span>연령대</span></a></li>
                            ...
                            <li class="col col-1-5" style="width: 18%"><a href="javascript:;" class="term-type-tab text-center" data-tab_type="Product"><span>상품</span></a></li>
                        ...
                        <table cellspacing="0" class="table-row table-a" style="margin-bottom: 0;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th><span>상태</span</th>
                                    <td>
                                        <input type="radio" name="searchStatus" id="searchStatus0" class="radio" <c:if test="${empty commandMap.searchStatus or commandMap.searchStatus eq Code.CONTRACT_STATUS_RECEIPT }">checked="checked"</c:if> value="<c:out value="${Code.CONTRACT_STATUS_RECEIPT }"/>" />
                                        <label for="searchStatus0">접수건수</label>
                                        <input type="radio" name="searchStatus" id="searchStatus1" class="radio" <c:if test="${commandMap.searchStatus eq Code.MEMBER_JOIN_READY }">checked="checked"</c:if> value="<c:out value="${Code.MEMBER_JOIN_READY }"/>" />
                                        <label for="searchStatus1">대기건수</label>
                                        <input type="radio" name="searchStatus" id="searchStatus2" class="radio" <c:if test="${commandMap.searchStatus eq Code.MEMBER_JOIN_COMPLETE }">checked="checked"</c:if> value="<c:out value="${Code.MEMBER_JOIN_COMPLETE }"/>" />
                                        <label for="searchStatus2">가입건수</label>
                                        <input type="radio" name="searchStatus" id="searchStatus3" class="radio" <c:if test="${commandMap.searchStatus eq Code.MEMBER_JOIN_CANCEL }">checked="checked"</c:if> value="<c:out value="${Code.MEMBER_JOIN_CANCEL }"/>" />
                                        <label for="searchStatus3">해약건수</label>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                            ...
                            <c:if test="${not empty list }">
                                <c:forEach var="row" items="${list }" varStatus="stat">
                                    <tr <c:if test="${stat.last }">class="total"</c:if>>
                                        ...
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.ONE_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.TWO_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.THREE_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FOUR_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FIVE_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.SIX_M }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.ONE_F }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.TWO_F }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.THREE_F }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FOUR_F }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.FIVE_F }" type="number"/></td>
                                        <td <c:if test="${stat.last }">class="col"</c:if>><fmt:formatNumber value="${row.SIX_F }" type="number"/></td>
                                    </tr>
                                </c:forEach>
                            </c:if>
    ...
    <script>
        $(function() {
            ...
            $wrapper.on("click", "#search", function() {
                ...
                $searchForm.postSubmit($.action.view("Age", "statsJoinAnalysis"));
            });
            <%-- 엑셀 다운로드 --%>
            $wrapper.on("click", "#goExcelDownload", function() {
                $frm.postSubmit($.action.exceldownload("Age", "statsJoinAnalysis"));
            });
        });
    </script>
</body>
