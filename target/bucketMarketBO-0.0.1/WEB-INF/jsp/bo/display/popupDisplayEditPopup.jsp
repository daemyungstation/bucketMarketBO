        ...
        <div class="modal-wrapper">
            <div class="modal-inner">
                <div class="push-guide instant">
                    <i class="icon"></i>
                    <h2 class="title">
                        <span><c:out value="${Const.DEVICE[info.POP_MST_CHN_GBN] }"/> 팝업 수정</span>
                    </h2>
                    <div class="text-left">
                        <small class="desc">
                ...
                <form id="frm" name="frm"  enctype="multipart/form-data">
                    <input type="hidden" name="POP_MST_IDX" value="<c:out value="${info.POP_MST_IDX }"/>"/>
                    <table cellspacing="0" class="table-row table-a">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <input type="radio" name="POP_MST_USE_YN" id="POP_MST_USE_YN0" class="radio" value="Y" <c:if test="${info.POP_MST_USE_YN eq 'Y' }">checked="checked"</c:if>/>
                                    <label for="POP_MST_USE_YN0">노출</label>
                                    <input type="radio" name="POP_MST_USE_YN" id="POP_MST_USE_YN1" class="radio" value="N" <c:if test="${info.POP_MST_USE_YN eq 'N' }">checked="checked"</c:if>/>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="POP_MST_TITLE" id="POP_MST_TITLE" class="text xlarge" value="<c:out value="${info.POP_MST_TITLE }"/>" maxlength="70" />
                                </td>
                            </tr>
                            <tr>
                                <th>
                                ...
                                <td>
                                    ...
                                    <input type="text" name="POP_MST_ST_DT" id="POP_MST_ST_DT" class="text date" value="<ui:formatDate value="${info.POP_MST_ST_DT }" pattern="yyyy-MM-dd"/>" data-target-end="#POP_MST_ED_DT" />
                                    <select name="POP_MST_ST_HH" id="POP_MST_ST_HH" class="select small">
                                        <c:forEach var="hh" begin="0" end="23" step="1">
                                            ...
                                            <c:out value="${hour }"/>
                                            <option value="<c:out value="${hour }"/>" <c:if test="${info.POP_MST_ST_HH eq hour }">selected="selected"</c:if>><c:out value="${hour }"/></option>
                                        </c:forEach>
                                    </select>
                                    <select name="POP_MST_ST_MM" id="POP_MST_ST_MM" class="select small">
                                        <c:forEach var="mm" begin="0" end="59" step="1">
                                            ...
                                            <option value="<c:out value="${minute }"/>" <c:if test="${info.POP_MST_ST_MM eq minute }">selected="selected"</c:if>><c:out value="${minute }"/></option>
                                        </c:forEach>
                                    </select>
                                    ~
                                    <a href="javascript:;" class="js-datepickr">
                                    ...
                                    <input type="text" name="POP_MST_ED_DT" id="POP_MST_ED_DT" class="text date" value="<ui:formatDate value="${info.POP_MST_ED_DT }" pattern="yyyy-MM-dd"/>" data-target-start="#POP_MST_ST_DT" />
                                    <select name="POP_MST_ED_HH" id="POP_MST_ED_HH" class="select small">
                                        <c:forEach var="hh" begin="0" end="23" step="1">
                                            ...
                                            <c:out value="${hour }"/>
                                            <option value="<c:out value="${hour }"/>" <c:if test="${info.POP_MST_ED_HH eq hour }">selected="selected"</c:if>><c:out value="${hour }"/></option>
                                        </c:forEach>
                                    </select>
                                    <select name="POP_MST_ED_MI" id="POP_MST_ED_MI" class="select small">
                                        <c:forEach var="mm" begin="0" end="59" step="1">
                                            ...
                                            <option value="<c:out value="${minute }"/>" <c:if test="${info.POP_MST_ED_MI eq minute }">selected="selected"</c:if>><c:out value="${minute }"/></option>
                                        </c:forEach>
                                    </select>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="radio" name="POP_MST_DAY_YN" id="POP_MST_DAY_YN0" class="radio" value="N" <c:if test="${info.POP_MST_DAY_YN eq 'N' }">checked="checked"</c:if>/>
                                    <label for="POP_MST_DAY_YN0">사용안함</label>
                                    <input type="radio" name="POP_MST_DAY_YN" id="POP_MST_DAY_YN1" class="radio" value="Y" <c:if test="${info.POP_MST_DAY_YN eq 'Y' }">checked="checked"</c:if>/>
                                    <label for="POP_MST_DAY_YN1">사용함</label>
                                </td>
                            </tr>
                            <c:if test="${info.POP_MST_CHN_GBN eq Const.PC }">
                                <tr>
                                    <th>
                                        <label class="label">
                                            <span>팝업창 크기</span>
                                    ...
                                    <td>
                                        ...
                                        <input type="text" name="POP_MST_W" id="POP_MST_W" class="text onlyNumber" value="<c:out value="${info.POP_MST_W }"/>" maxlength="4"/> PX
                                        <label for="POP_MST_H">세로 </label>
                                        <input type="text" name="POP_MST_H" id="POP_MST_H" class="text onlyNumber" value="<c:out value="${info.POP_MST_H }"/>" maxlength="4"/> PX
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                                    ...
                                    <td>
                                        ...
                                        <input type="text" name="POP_MST_X" id="POP_MST_X" class="text onlyNumber" value="<c:out value="${info.POP_MST_X }"/>" maxlength="4"/> PX 이동,
                                        <label for="POP_MST_Y">화면 위쪽에서 </label>
                                        <input type="text" name="POP_MST_Y" id="POP_MST_Y" class="text onlyNumber" value="<c:out value="${info.POP_MST_Y }"/>" maxlength="4"/> PX 만큼 아래에 위치합니다.
                                        <small class="desc">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <jsp:include page="/WEB-INF/jsp/common/template/file.jsp" flush="false">
                                        ...
                                        <jsp:param name="fileType" value="${info.POP_MST_CHN_GBN }"/>
                                    </jsp:include>
                                    <c:if test="${info.POP_MST_CHN_GBN eq Const.MOBILE }">
                                        <p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <c:forEach var="target" items="${Const.LINK_TARGET }" varStatus="i">
                                        <input type="radio" name="POP_MST_MDE" id="POP_MST_MDE<c:out value="${i.index }"/>" value="<c:out value="${target.key }"/>" <c:if test="${info.POP_MST_MDE eq target.key }">checked="checked"</c:if>/>
                                        <label for="POP_MST_MDE<c:out value="${i.index }"/>"><c:out value="${target.value }"/></label>
                                    </c:forEach>
                                    <br/>
                                    <input type="text" placeholder="링크 입력 시 https://를 포함하여 전체 링크를 입력해 주세요"  name="POP_MST_LNK" id="POP_MST_LNK" class="text xlarge" value="<c:out value="${info.POP_MST_LNK }"/>" maxlength="100" <c:if test="${info.POP_MST_MDE eq Const.LINK_TARGET_NONE }">disabled="disabled"</c:if>/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
